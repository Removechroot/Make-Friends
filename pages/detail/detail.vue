	...
	<view v-if="IsShow">
			...
			<view class="">
				<view>
					<block v-if="info.titlepic.length > 0">
						<image :src="item.imgUrl" mode="" @click="preview(index)" class="w-100" mode="aspectFit" v-for="(item, index) in info.titlepic" :key="index"></image>
					</block>
				</view>
			</view>
		</common-list>
		...
		<view class="px-2">
			<template v-if="review.length > 0">
				<view class="uni-comment-list" v-for="(item, index) in review" :key="index">
			...
			<template v-else>
				<nothing><view>抢沙发你是第一个！加油</view></nothing>
			</template>
		</view>
...
export default {
	...
	onShareAppMessage() {
		return {
			title: '自定义分享标题',
			path: '/pages/detail/detail?detail='+ this.info._id
		};
	},
	onLoad(e) {
		// 初始化
	...
	methods: {
		...
		submit(data) {
			...
			let obj = {
				user_id: 1,
				avatar: 'https://vkceyugu.cdn.bspapp.com/VKCEYUGU-mf-ser1/98883de0-093e-11eb-8a36-ebb87efcf8c0.jpg',
				username: '测试用户',
				msg: data,
				type: 'text',
